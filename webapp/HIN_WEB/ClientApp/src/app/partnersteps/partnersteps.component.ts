...
@Component({
  selector: 'app-partnersteps',
  templateUrl: './partnersteps.component.html',
  styleUrls: ['./partnersteps.component.css']
})
export class PartnerstepsComponent implements OnInit {
  ...
  public entityType: number = LeadGenType.Partner;
  ...
  ngOnInit() {
    ...
    if (partnerData) {
      ...
      this.entityId = this.partner.PartnerId;
      this.entityNumber = this.partner.PartnerNumber;
      this.getPipeLine(this.partner.PipelineGroupId);
      this.LoadNotes(this.partner.LeadId, this.entityType);
    }
    else {
  ...
  getContactInformation() {
    this.contactService.getContactInformation(this.entityType, this.partner.PartnerId, false).subscribe(data => {
      if (data != null) {
  ...
  getPipeLine(id) {
    if (id) {
      this.dealService.getPipeLineByPipeLineGroupId(id).subscribe(data => {
        ...
        this.selectedPipeIndex = this.listofPipeline.findIndex(x => x.PipelineId == this.partner.PipelineId);
        if (this.selectedPipeIndex == -1) {
          this.selectedPipeIndex = 0;
          this.partner.PipelineId = this.listofPipeline[this.selectedPipeIndex].PipelineId;
        }
      });
    }
  }
  ...
  saveContactInfo(contactInfo) {
    ...
    contactInfo.EntityId = this.partner.PartnerId;
    if (!contactInfo.Id) {
  ...
  cancel() {
    ...
    this.dataShared.setValue(this.partner.PipelineGroupId);
