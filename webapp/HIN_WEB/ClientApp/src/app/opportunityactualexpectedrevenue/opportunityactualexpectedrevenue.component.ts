...
@Component({
  selector: 'app-opportunityactualexpectedrevenue',
  templateUrl: './opportunityactualexpectedrevenue.component.html',
  styleUrls: ['./opportunityactualexpectedrevenue.component.css']
})
export class OpportunityactualexpectedrevenueComponent implements OnInit {
  ...
  calculateTotalVarianceAmountPer(listDeal) {
    let totalOpportunityAmount = 0;
    listDeal.forEach(data => {
      totalOpportunityAmount += (((data.ActualAmount) / (data.ExpectedRevenue) - 1) * 100);
    });
    return totalOpportunityAmount;
  }
  calculateExpectedSales(listDeal) {
    let totalOpportunityAmount = 0;
    listDeal.forEach(data => {
      totalOpportunityAmount += this.parseInteger((data.ExpectedRevenue) * (data.Percentage) / 100);
    });
    return totalOpportunityAmount;
  }
  ...
  exportCSV() {
    ...
    console.log(csv)
    this.saveBlob(csv, "Opp_ActualAndExpectedSalesReport.csv");
  }
  ...
  parseOppCostList(lstDeal) {
    ...
    for (let i = 0; i < lstDeal.length; i++) {
      ...
      _oppCost.SalesFunnel = lstDeal[i].StatusId == 1 ? 100 : (lstDeal[i].Percentage);
      ...
      _oppCost.VarianceAmountPercentage = (((this.parseNumber(lstDeal[i].ActualAmount) / (lstDeal[i].ExpectedRevenue)) - 1) * 100);
      _oppCost.ExpectedSalesCalculation = lstDeal[i].StatusId == 1 ? (lstDeal[i].ActualAmount) : (((this.parseNumber(lstDeal[i].ExpectedRevenue) * (lstDeal[i].Percentage)) / 100));
