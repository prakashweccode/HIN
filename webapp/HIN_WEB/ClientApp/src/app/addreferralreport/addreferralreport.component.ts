...
@Component({
  selector: 'app-addreferralreport',
  templateUrl: './addreferralreport.component.html',
  styleUrls: ['./addreferralreport.component.css']
})
export class AddreferralreportComponent implements OnInit {
  ...
  constructor(public router: Router, public addReferralReportService: AddreferralreportService, private notyHelper: NotyHelper) { }
  ...
  getLeadReportByReferral() {
    ...
    this.addReferralReportService.getLeadReportByReferral(this.opportunityacquisitioncost).subscribe(data => {
      if (data != null)
        this.listLead = data;
    }, err => {
  ...
  exportCSV() {
    ...
    console.log(csv)
    this.saveBlob(csv, "LeadReportByReferral.csv");
  }
