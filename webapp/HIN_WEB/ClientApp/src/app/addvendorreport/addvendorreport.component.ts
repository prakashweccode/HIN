...
@Component({
  selector: 'app-addvendorreport',
  templateUrl: './addvendorreport.component.html',
  styleUrls: ['./addvendorreport.component.css']
})
export class AddvendorreportComponent implements OnInit {
  ...
  constructor(public router: Router, public addVenodrReportService: AddvendorreportService,private notyHelper: NotyHelper) { }
  ...
  handleFrequencyChange(evt) {
    if (evt.target.checked) {
      ...
      this.leadReport.reportinput.From = new Date();
      this.leadReport.reportinput.To = new Date();
      this.listLead = [];
    }
  }
  ...
  getLeadReportByVendor() {
    ...
    this.addVenodrReportService.getLeadReportByVendor(this.opportunityacquisitioncost).subscribe(data => {
        if (data != null)
          this.listLead = data;
      }, err => {
  ...
  exportCSV() {
    ...
    console.log(csv)
    this.saveBlob(csv, "LeadReportByVendor.csv");
  }
