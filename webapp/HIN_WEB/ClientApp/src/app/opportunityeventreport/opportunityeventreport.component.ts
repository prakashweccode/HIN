...
@Component({
  selector: 'app-opportunityeventreport',
  templateUrl: './opportunityeventreport.component.html',
  styleUrls: ['./opportunityeventreport.component.css']
})
export class OpportunityeventreportComponent implements OnInit {
  ...
  constructor(public router: Router, public opportunityEventReportService: OpportunityeventreportService, private notyHelper: NotyHelper) { }
  ...
  getOpportunitiesByEvent() {
    if (!this.deal.AssignedTo) {
      ...
      this.opportunityEventReportService.getOpportunitiesByEvent(this.changeFormatDate(this.dealReport.reportinput.From), this.changeFormatDate(this.dealReport.reportinput.To), this.deal.AssignedTo).subscribe(data => {
        if (data != null)
          this.listDeal = data;
      }, err => {
  ...
  selectAssignedValue(evt) {
    this.deal.AssignedTo = evt.Id;
    this.deal.AssignedName = evt.Name;

  }

  getAssignedToGridData() {
    this.assignedToGrid.ApiUrl = "/api/EventShow/GetEventShow";
