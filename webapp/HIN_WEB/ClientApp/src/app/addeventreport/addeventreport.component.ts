...
@Component({
  selector: 'app-addeventreport',
  templateUrl: './addeventreport.component.html',
  styleUrls: ['./addeventreport.component.css']
})
export class AddeventreportComponent implements OnInit {
  ...
  constructor(public router: Router, public addEventReportService: AddeventreportService, private notyHelper: NotyHelper) { }
  ...
  getLeadReportByEvent() {
    ...
    //else {
      ...
      this.addEventReportService.getLeadReportByEvent(this.opportunityacquisitioncost).subscribe(data => {
        if (data != null)
          this.listLead = data;
      }, err => {
  ...
  exportCSV() {
    ...
    console.log(csv)
    this.saveBlob(csv, "LeadReportByEvent.csv");
  }
